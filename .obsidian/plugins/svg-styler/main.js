...
var require_svgson_umd = __commonJS({
  "node_modules/svgson/dist/svgson.umd.js"(exports, module2) {
    (function(global, factory) {
      typeof exports === "object" && typeof module2 !== "undefined" ? module2.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.svgson = factory());
    })(exports, function() {
      "use strict";
      ...
      var renameKeys = createCommonjsModule(function(module3) {
        (function() {
      ...
      var eventemitter3 = createCommonjsModule(function(module3) {
...
__export(main_exports, {
  default: () => SvgStyleEditor
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");
var svgson = __toESM(require_svgson_umd());
var css = __toESM(require_css());

// src/presets.ts
var generate_default_preset = function() {
  return {
    circle: {
      fill: {
        name: "Fill Color",
...
var generate_global_svg_preset = function() {
  return {
    width: {
      name: "Width",
      key: "width",
...
var shrink_style_preset = function(preset) {
  const output = {};
  for (const tag_name of Object.keys(preset)) {
    ...
    for (const prop_name of Object.keys(tag)) {
      ...
      if (prop.newValue != null && prop.newValue != void 0 && prop.newValue != prop.currentValue) {
        tag_obj[prop_name] = prop.newValue;
        continue;
      }
      if ((prop.newValue == null || prop.newValue == void 0) && prop.currentValue != null) {
...
var srhink_global_preset = function(preset) {
  let output = {};
...
var SvgStyleEditor = class extends import_obsidian.Plugin {
  async onload() {
    print(this.app);
    this.loadSettings();
    ...
    this.addCommand({
      ...
      editorCheckCallback: (checking, editor, view) => {
        ...
        if (checking) {
          ...
          const linkedFile = this.app.metadataCache.getFirstLinkpathDest(filePath, (activeFile == null ? void 0 : activeFile.path) || "");
          print(linkedFile);
          if (linkedFile && linkedFile.extension === "svg") {
            new SvgStyleEditorModal(this.app, this.settings, linkedFile, editor).open();
          } else {
            new import_obsidian.Notice("SVG file not found: " + filePath);
          }
        }
      }
    });
...
var SvgStylerSettingPage = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    const drop_down_desc = this.plugin.settings.duplicateSvgDirectory == "default" ? 'You can change the default resource path in Obsidian "Settings > Files and Links > Attachment folder path"' : "Please provide a valid directory path bellow";
    new import_obsidian.Setting(containerEl).setName("Duplicate SVG directory").setHeading();
    containerEl.createEl("p", {
      text: "Choose where duplicate SVG files should be saved. You can select the default Obsidian resource directory or specify a custom directory."
    });
    new import_obsidian.Setting(containerEl).setName("Duplicate SVG location").setDesc(drop_down_desc).addDropdown((dropdown) => {
      dropdown.addOption("default", "Default resource/attachment directory").addOption("custom", "Custom directory").setValue(this.plugin.settings.duplicateSvgDirectory).onChange(async (value) => {
    ...
    if (this.plugin.settings.duplicateSvgDirectory === "custom") {
      new import_obsidian.Setting(containerEl).setName("Custom directory path").setDesc("Enter the full path for the custom directory. A directory will be created if the path does not exits").addText((text) => {
        text.setPlaceholder("/path/to/directory").setValue(this.plugin.settings.customDirectoryPath).onChange(async (value) => {
...
var SvgStyleEditorModal = class extends import_obsidian.Modal {
  constructor(app, setting, file, editor) {
    super(app);
  ...
  findDrawingTags(svgsonObj, allowed_elements) {
    let foundTags = /* @__PURE__ */ new Set();
  ...
  async processSvg() {
    ...
    if (!hasSvgTag) {
      new import_obsidian.Notice(`${this.file} has no <svg> </svg> pair tags!`);
      this.close();
    }
    ...
    for (const attr of Object.keys(input_svg.attributes)) {
      if (editable_global_svg_attributes.includes(attr)) {
        this.global_preset[attr].currentValue = input_svg.attributes[attr];
      } else {
    ...
    if (input_style_tag && input_style_tag.children.length > 0) {
      ...
      is_style_tag_valid = input_style_tag_css && input_style_tag_css.stylesheet && input_style_tag_css.stylesheet.rules.length > 0;
    }
      ...
      for (const rule of input_style_tag_css.stylesheet.rules) {
        ...
        for (let selector of rule.selectors) {
          ...
          for (let decl of rule.declarations) {
            let decl_prop = this.style_prest[selector][decl.property];
            if (!decl_prop) {
              print(`>>> Property is not supported: ${decl.property}`);
              continue;
            }
            this.style_prest[selector][decl.property].currentValue = decl.value;
          }
        }
      }
    }
  ...
  async onOpen() {
    ...
    for (const attribute of Object.keys(this.global_preset)) {
      ...
      if (attr_obj.type == "float") {
        element = new import_obsidian.Setting(elem_detail_global).setName(setting_label).addSlider((cb) => {
          cb.setLimits(0, 1, 0.01).setValue(
            _m ? Number(attr_obj.placeholder) : Number(attr_obj.currentValue)
          ).onChange((value) => {
            attr_obj.newValue = `${value}`.trim();
          });
        });
      } else {
        new import_obsidian.Setting(elem_detail_global).setName(setting_label).addText((text) => {
          text.setPlaceholder(attr_obj.placeholder).setValue(_m ? "" : `${attr_obj.currentValue}`).onChange((value) => {
            if ((value == null ? void 0 : value.trim().length) == 0) {
    ...
    for (const tag_name of this.available_drawing_tags) {
      ...
      for (const prop_name of Object.keys(tag_obj)) {
        ...
        const setting_label = _m ? prop_obj.name : `${prop_obj.name} (\u270F\uFE0F)`;
        if (prop_obj.type == "color") {
          new import_obsidian.Setting(elem_detail_style).setName(setting_label).addColorPicker((cb) => {
            cb.setValue(
              _m ? prop_obj.placeholder : prop_obj.currentValue
            ).onChange((hex_str) => {
              prop_obj.newValue = hex_str;
            });
          });
        } else if (prop_obj.type == "float") {
          new import_obsidian.Setting(elem_detail_style).setName(setting_label).addSlider((cb) => {
            cb.setLimits(0, 1, 0.01).setValue(
              _m ? Number(prop_obj.placeholder) : Number(prop_obj.currentValue)
            ).onChange((value) => {
          ...
          new import_obsidian.Setting(elem_detail_style).setName(setting_label).addDropdown((cb) => {
            cb.addOptions(_options).setValue(
              _m ? prop_obj.placeholder : prop_obj.currentValue
            ).onChange((value) => {
          ...
          new import_obsidian.Setting(elem_detail_style).setName(setting_label).addText((cb) => {
            cb.setPlaceholder(prop_obj.placeholder).setValue(_m ? "" : prop_obj.currentValue).onChange((value) => {
              if ((value == null ? void 0 : value.trim().length) == 0) {
    ...
    contentEl.createDiv().addClass("asvgs_hr_divider");
    const _setting_buttons = new import_obsidian.Setting(contentEl).addButton((btn) => {
      btn.setButtonText("Cancel").onClick(async () => {
        this.close();
      });
    }).addButton((btn) => {
      btn.setButtonText("Save as duplicate").setCta().onClick(async () => {
        await this.modifySvg(true);
        this.close();
      });
    }).addButton((btn) => {
      btn.setButtonText("Save in-place").setCta().onClick(async () => {
    ...
    if (this.has_valid_style_tag) {
      _setting_buttons.addButton((btn) => {
        btn.setButtonText("Clear styles").setWarning().onClick(async () => {
    ...
    if (DEBUG_MODE) {
      _setting_buttons.addButton((btn) => {
        btn.setButtonText("DEBUG").setWarning().onClick(() => {
  ...
  async modifySvg(duplicate) {
    try {
      const globals = srhink_global_preset(this.global_preset);
      print(globals);
      this.input_svg.attributes = globals;
      const styles = shrink_style_preset(this.style_prest);
      print(styles);
      let style_obj = {
        type: "stylesheet",
        stylesheet: {
...
`,
      ...
      if (error instanceof Error) {
        ...
        new import_obsidian.Notice("Error modifying SVG file: " + error.message);
      } else {
        new import_obsidian.Notice("Error modifying SVG file.");
      }
    }
  }
  async saveSvgToFile(duplicate, svg_str_content) {
    function getRandomInt(max) {
      return Math.floor(Math.random() * max);
    }
    if (!duplicate) {
      this.app.vault.modify(this.file, svg_str_content);
      new import_obsidian.Notice("SVG styles updated successfully.");
    } else {
      ...
      if (this.plugin_setting.duplicateSvgDirectory == "default") {
        ...
        const customPath = (0, import_obsidian.normalizePath)(this.plugin_setting.customDirectoryPath);
        const vaultBasePath = this.app.vault.adapter.basePath;
        const fullCustomPath = (0, import_obsidian.normalizePath)(vaultBasePath + "/" + customPath);
        const directoryExists = await this.app.vault.adapter.exists(fullCustomPath);
        if (!directoryExists) {
          try {
            await this.app.vault.adapter.mkdir(customPath);
          } catch (error) {
            new import_obsidian.Notice("Failed to find or create custom directory. Please enter a valid path.");
      ...
      new import_obsidian.Notice("SVG style duplicated successfully: " + duplicate_path);
  ...
  async clearStyles() {
    try {
      ...
      new import_obsidian.Notice("SVG styles cleared successfully.");
      this.refreshEditor();
    } catch (error) {
      if (error instanceof Error) {
        print("Error: ", error.message);
        ...
        new import_obsidian.Notice("Error clearing styles from the svg file.");
      }
    }
  }
  refreshEditor() {
    var _a;
    (_a = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView)) == null ? void 0 : _a.leaf.rebuildView();
  }
  onClose() {
    const { contentEl } = this;
    contentEl.empty();
    this.input_svg = void 0;
    this.style_prest = void 0;
    this.global_preset = void 0;
  }
};
